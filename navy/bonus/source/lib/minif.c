...
size_t minif(const char * const str, ...)
{
    ...
    size_t j = 0;
    size_t len = 0;
    char flags[] = {'c', 'd', 'i', 's', 'u', 0};
    ptr fct[] = {&print_list_char, &print_list_number, &print_list_number,
    ...
    for (size_t i = 0; str[i]; i++)
        if (str[i] == '%') {
            while (str[++i] == ' ');
            for (j = 0; flags[j] && str[i] != flags[j]; j++);
            len += (!flags[j]) ? (size_t) write(1, &str[i], 1) : fct[j](list);
        } else
            len += write(1, str + i, 1);
    va_end(list);
    return (len);
